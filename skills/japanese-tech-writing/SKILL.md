...
when_to_use: 日本語で技術書の章、草稿、記事、解説文、README などを書くとき、または推敲・リライトするとき。Markdown ドキュメントの文章を整えるときにも使う。トリガー例:「この文章を推敲して」「README を読みやすくして」「日本語の技術文書を書いて」。
---
...
- 出力先が Markdown（GitHub Flavored Markdown など）のときは、ソースの改行がそのままでは表示の改行にならない点に注意する。素の改行は半角スペースに畳まれ、表示では1段落につながる。1文1行を表示にも反映するなら、各行末に半角スペース2つ（ハードブレーク）を付けるか、空行で段落を分ける。表示を分けないなら、1文1行にせず散文段落のまま書く。同じ文書内で方式を統一し、ソースだけ改行されて表示は1段落、という中間状態を残さない。
...
- かっこ書きの中身は「具体的な値や例」「正式名称と略称の定義」「出典や参照先」に限定する。ニュアンスの言い換えや評価の補足（「（一時的な値）」「（重要）」「（推奨）」など）には使わない。
- かっこ書きに体言止めの曖昧な補足を置かない。補足は主語と対象を省かず文として完結させるか、本文に溶かす。かっこの中身だけを読んで主語と対象が分かるかを点検し、分からなければ削除するか文にする。
...
要件・仕様・タスクなど、項目を列挙する箇条書きでは、次のように書く。

- トップレベルの項目は、名詞・体言止め・短い語句で書き、完全な文にしない。
- 理由・補足・背景などの説明は、その項目の下にインデントした子項目に書く。
- トップレベルの項目に説明的な文を混ぜ込まない。説明は子項目に分ける。
- 説明が不要なほど自明な項目には、子項目を付けない。
- 一つの項目に複数の情報を詰め込まない。

悪い例（項目に説明が混ざっている）：

- ユーザー認証を実装する。メールとパスワードで登録・ログインでき、パスワードはハッシュ化して保存する必要がある。

良い例（項目は短く、説明は子項目に分ける）：

- ユーザー認証（メール＋パスワード）
  - 平文保存は避け、パスワードはハッシュ化して保存する
  - 登録・ログインの両フローが必要
- レート制限
  - 総当たり攻撃を防ぐため、ログイン試行回数に上限を設ける
